...
from data.cnews_loader import read_vocab, read_category, batch_iter, process_file, build_vocab,get_embedding
...
def evaluate(sess,cur,source='tb_test_text'):
    """评估在某一数据上的准确率和损失"""
...
def train():
    ...
    for epoch in range(config.num_epochs):
        print('Epoch:', epoch + 1)
        batch_train = batch_iter(cur,batch_size=config.batch_size,source='tb_train_text')
        for x_batch, y_batch in batch_train:
            ...
            if total_batch % config.print_per_batch == 0:
                ...
                loss_val, acc_val = evaluate(session, cur,source='tb_test_text')  # todo
